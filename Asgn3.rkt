...
(define tempFds ( list (FdC 'f 'x (BinopC '+ (NumC 2) (IdC 'x))) (FdC 'g 'y (BinopC '+ (NumC 5) (IdC 'y)))))
;(define fds (list))
...
(check-equal? (parse-prog '{{def {double x} = {* x 2}}
                            {def {tri f} = {* f 3}}
                            {def {main init} = {double 7}}}) (list (FdC 'double 'x (BinopC '* (IdC 'x) (NumC 2))) (FdC 'tri 'f (BinopC '* (IdC 'f) (NumC 3))) (FdC 'main 'init (AppC 'double (NumC 7))) ))
(check-equal? (parse-prog '{{def {double x} = {* x 2}}
...
(check-equal? (interp (NumC 1) tempFds) 1)
(check-equal? (interp (NumC 0) tempFds) 0)
(check-equal? (interp (BinopC '+ (NumC 1) (NumC 2)) tempFds) 3)
(check-equal? (interp (BinopC '* (NumC 1) (NumC 2)) tempFds) 2)
(check-equal? (interp (BinopC '* (BinopC '+ (NumC 1) (NumC 10)) (NumC 2)) tempFds) 22)
#;[
...
(check-equal? (interp (parse '(* 3 (+ 2 7))) tempFds) 27)
...
(define (interp-fns [funs : (Listof FdC)]) : Real
  (printf "f1: ~v~n" funs)
  (define mainFunc (get-fundef 'main funs))
  (printf "f2: ~v~n" funs)
  (interp (FdC-body mainFunc) funs)
  
  ) 

(parse-prog '{{def {double x} = {* 2 x}}
                            {def {main init} = {double 13}}})
